...
export function FaqPanel({ appSettings, onUpdateSettings, tickets = [] }: FaqPanelProps) {
  ...
  const [aiNumberAndTitle, setAiNumberAndTitle] = useState<string | null>(null);
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.matchedFaqId) {
          ...
          setAiNumberAndTitle(data.numberAndTitle || (matched ? (matched.faqNumber ? `FAQ# ${matched.faqNumber} — ${matched.name}` : matched.name) : null));
        } else {
          setAiMatchedFaq(null);
          setAiNumberAndTitle(null);
        }
              ...
              <div className="space-y-3">
                <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-center justify-between shadow-xs">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[11px] font-bold text-emerald-700 uppercase tracking-wider mb-1">FAQ Encontrada pela IA</p>
                      <h4 className="text-base font-black text-slate-900">{aiNumberAndTitle}</h4>
                    </div>
                  </div>
