...
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, BarChart, Bar, Legend, Cell, ReferenceArea } from 'recharts';
...
export function SLADashboard({ tickets, appSettings }: SLADashboardProps) {
  const [timeFilter, setTimeFilter] = useState<TimeFilter>(appSettings.defaultSlaTimeFilter || 'month');
  ...
  const finishedTickets = useMemo(() => {
    return tickets.filter(t => t.status === 'FINALIZADO' && !t.deleted && !t.archived);
  }, [tickets]);
  ...
  const filteredTickets = useMemo(() => {
    return finishedTickets.filter(t => {
      ...
      if (timeFilter === 'week' && (diffDays < 0 || diffDays > 7)) return false;
      if (timeFilter === 'month' && format(date, 'yyyy-MM') !== selectedMonth) return false;
      if (timeFilter === 'year' && (diffDays < 0 || diffDays > 365)) return false;
      
      if (selectedCategory !== 'all' && t.category !== selectedCategory) return false;

      return true;
    });
  }, [finishedTickets, timeFilter, selectedDate, selectedMonth, selectedCategory]);
  ...
  }, [finishedTickets, timeFilter, selectedDate, selectedMonth]);
  ...
  const maxTimelineVal = useMemo(() => {
    if (timelineData.length === 0) return 20;
    const max = Math.max(...timelineData.map(d => d.avgMinutes), 18);
            ...
            <div className="flex-1 w-full min-h-0">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={timelineData} margin={{ top: 10, right: 30, left: -20, bottom: 20 }}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                  <ReferenceArea {...({ y1: 0, y2: 7, fill: '#10b981', fillOpacity: 0.12 } as any)} />
                  <ReferenceArea {...({ y1: 7, y2: 9, fill: '#3b82f6', fillOpacity: 0.12 } as any)} />
                  <ReferenceArea {...({ y1: 9, y2: 15, fill: '#f97316', fillOpacity: 0.12 } as any)} />
                  <ReferenceArea {...({ y1: 15, y2: maxTimelineVal, fill: '#ef4444', fillOpacity: 0.12 } as any)} />
                  ...
                  <Line 
                    ...
                    dot={(props: any) => {
                      ...
                      const val = payload.avgMinutes;
                      let color = '#10b981';
                      if (val >= 15) color = '#ef4444';
                      else if (val >= 9) color = '#f97316';
                      else if (val >= 7) color = '#3b82f6';
                      return <circle key={`dot-${cx}-${cy}`} cx={cx} cy={cy} r={4} fill={color} stroke="#ffffff" strokeWidth={2} />;
                    }} 
                    activeDot={{ r: 6 }} 
                  />
                </LineChart>
