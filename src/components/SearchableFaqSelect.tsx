...
export function SearchableFaqSelect({ value, onChange, faqs = [], variant = 'badge' }: SearchableFaqSelectProps) {
  ...
  return (
    <div className={cn("relative", variant !== 'badge' ? "w-full" : "inline-block w-full")} ref={containerRef}>
      <button
        type="button"
        onClick={(e) => {
          e.stopPropagation();
      ...
      {isOpen && (
        <div 
          ...
          className={cn(
            ...
            variant === 'badge' ? "w-[320px] left-0 sm:left-auto sm:right-0" : "w-full min-w-[320px] left-0 sm:left-auto sm:right-0"
          )}
        >
          {/* Search Header */}
            ...
            {filteredFaqs.length === 0 ? (
              ...
              filteredFaqs.map(faq => (
                <button
                  ...
                  className={cn(
                    "w-full text-left pl-9 pr-4 py-2 text-xs flex flex-col gap-0.5 transition-colors hover:bg-slate-50 relative",
                ...
                >
                  <div className="flex items-center justify-between gap-2 w-full">
                    ...
                    {value === faq.id && <Check className="h-3 w-3 text-purple-600 shrink-0 absolute right-4 top-2" />}
                  </div>
                  <span className="line-clamp-2 text-slate-600 leading-normal pr-4">
                    {faq.name}
                  </span>
                  {faq.category && (
