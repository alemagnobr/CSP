...
const getModelFriendlyName = (modelId?: string) => {
  switch (modelId) {
    case 'openrouter/free': return 'OpenRouter Auto (Grátis)';
    case 'google/gemma-4-31b-it:free': return 'Gemma 4 31B';
    case 'nvidia/nemotron-3-super-120b-a12b:free': return 'Nemotron 3 120B';
    case 'inclusionai/ling-3.0-flash:free': return 'Ling 3.0 Flash';
    case 'poolside/laguna-s-2.1:free': return 'Laguna S 2.1';
    default:
      return 'Modelo Grátis';
  }
};
...
"{description}"`;
  ...
  const handleArchiveTicket = async (id: string) => {
    if (!user) return;
    try {
      await setDoc(doc(db, 'tickets', id), { archived: true }, { merge: true }); 
    } catch (error) {
  ...
  const handleNewTicket = () => {
    ...
    const newTicket: ActiveTicket = {
      tabId,
      id: '', // Blank by default so user enters real ticket number
      status: 'EM_ANDAMENTO',
      description: '',
      networkLogin: '',
      extension: '',
  ...
  const handleDuplicateTicket = (sourceTicket: ActiveTicket) => {
    ...
    const newTicket: ActiveTicket = {
      ...sourceTicket,
      tabId,
      id: '', // Blank by default for the new ticket
      description: '',
      category: '',
      durationSeconds: 0,
      status: 'EM_ANDAMENTO',
  ...
  const handleEditTicket = (id: string) => {
    ...
    const tabId = ticketToEdit.id;

    setActiveTabs(prev => {
      const alreadyOpen = prev.find(t => t.id === id || t.tabId === tabId);
  ...
  const handleCloseTab = (e: MouseEvent, tabIdentifier: string) => {
    e.stopPropagation();
    setActiveTabs(prev => {
      const newTabs = prev.filter(t => (t.tabId || t.id) !== tabIdentifier);
      if (currentTabId === tabIdentifier) {
        const nextKey = newTabs.length > 0 ? (newTabs[newTabs.length - 1].tabId || newTabs[newTabs.length - 1].id) : 'dashboard';
        setCurrentTabId(nextKey);
      }
      return newTabs;
    });
  };
  ...
  const handleFinishTicket = async (ticketToFinish: ActiveTicket) => {
    if (!ticketToFinish.id || !ticketToFinish.id.trim()) {
      return;
    }
    const cleanId = ticketToFinish.id.trim();
    const { isDraft, tabId, ...rest } = ticketToFinish;
    ...
    // Close the tab
    setActiveTabs(prev => {
      const newTabs = prev.filter(t => (t.tabId || t.id) !== (ticketToFinish.tabId || ticketToFinish.id));
      if (currentTabId === (ticketToFinish.tabId || ticketToFinish.id)) {
        const nextKey = newTabs.length > 0 ? (newTabs[newTabs.length - 1].tabId || newTabs[newTabs.length - 1].id) : 'dashboard';
        setCurrentTabId(nextKey);
      }
      return newTabs;
  ...
  const currentActiveTicket = activeTabs.find(t => (t.tabId || t.id) === currentTabId);
          ...
          {activeSidebarItem === 'Atendimento' && (
            <div className="flex items-end px-8 gap-1 overflow-x-auto bg-slate-50/80 pt-2 border-t border-slate-100">
              ...
              {activeTabs.map(tab => {
                ...
                const isSelected = currentTabId === tabKey;
                return (
                  <div
                    key={tabKey}
                    onClick={() => setCurrentTabId(tabKey)}
                    className={cn(
                  ...
                  >
                    {tab.id?.trim() ? `Chamado ${tab.id}` : 'Novo Chamado'}
                    <button 
            ...
            <div className="p-6 space-y-4">
              ...
              {appSettings.aiProvider === 'openrouter' && (
                <div className="mt-4 pt-4 border-t border-slate-100 space-y-2">
                  ...
                  <select
                    value={appSettings.openRouterModel || 'openrouter/free'}
                    onChange={(e) => {
